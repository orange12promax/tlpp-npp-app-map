...
RCT_EXPORT_METHOD(reverseGeoCode:(double)lat lng:(double)lng) {
    ...
    if(flag) {
        ...
        [self decompileAddress:lat lng:lng];
    }
}
...
- (void)decompileAddress:(double)lat lng:(double)lng
{
    CLLocation *newLocation = [[CLLocation alloc] initWithLatitude:lat longitude:lng];
    ...
    [[self geocoder] reverseGeocodeLocation:newLocation completionHandler:^(NSArray*placemarks,NSError* error) {
        if (placemarks && [placemarks count] > 0) {
            ...
            NSMutableDictionary *body = [self getEmptyBody];
            //                body[@"cityCode"] = @(searchRecord.cityID).stringValue;
            body[@"latitude"] = [NSString stringWithFormat:@"%lf", lat];
            body[@"longitude"] = [NSString stringWithFormat:@"%lf", lng];
            body[@"address"] = [[mark.addressDictionary valueForKey:@"FormattedAddressLines"]componentsJoinedByString:@", "];;
            body[@"province"] = mark.administrativeArea;
            NSString *city = mark.locality;
            body[@"city"] = city ? city : mark.administrativeArea;
            body[@"district"] = mark.subLocality;
            body[@"streetName"] = mark.thoroughfare;
            body[@"streetNumber"] = mark.subThoroughfare;
            
            [weakSelf sendEvent:@"onGetReverseGeoCodeResult" body:body];
        }
    }];
}
